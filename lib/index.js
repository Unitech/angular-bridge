...
AngularBridge.prototype.collectionGet = function() { 
	return _.bind(function(req, res, next) {
		...
		if (req.resource.options == null || typeof req.resource.options['query'] == 'undefined') {
			...
			if(typeof req.resource.options['query'] == 'string') var dbQueryOptions = eval('(' + req.resource.options.query + ')');
			else if(typeof req.resource.options['query'] == 'object') var dbQueryOptions = req.resource.options.query;
			else var dbQueryOptions = {};
		}
		...
		query.exec(function(err, docs) {
			if (err) {
				...
				if(req.resource.model.schema.methods.query) req.resource.model.schema.methods.query();
				res.send(docs);
			}
		});
	}, this); 
...
AngularBridge.prototype.epureRequest = function(req_data, resource) {
	...
	if (resource.options == null || (typeof resource.options['hide'] == 'undefined' && typeof resource.options['readOnly'] == 'undefined'))
	return req_data;
...
AngularBridge.prototype.entityGet = function() {     
	return _.bind(function(req, res, next) {
		...
		if(req.doc.schema.methods.get) req.doc.schema.methods.get();
...
AngularBridge.prototype.entityPut = function() { 
	return _.bind(function(req, res, next) {
		...
		req.doc.save(function(err) {
			...
			if(req.doc.schema.methods.put) req.doc.schema.methods.put();
			return res.send(req.doc);
		});
...
AngularBridge.prototype.entityDelete = function() { 
	return _.bind(function(req, res, next) {
		...
		req.doc.remove(function(err) {
			...
			if(req.doc.schema.methods.delete) req.doc.schema.methods.delete();
