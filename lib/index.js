...
var AngularBridge = function(router, options) {
  this.router = router;
...
/**
 * Registers all REST routes with the provided `router` object.
 */
AngularBridge.prototype.registerRoutes = function() {
  this.router.route(this.options.urlPrefix + ':resourceName')
    .all(this.options.requestPrehandler)
    .all(this.collection())
    .get(this.collectionGet())
    .post(this.collectionPost())

  this.router.route(this.options.urlPrefix + ':resourceName/:id')
    .all(this.options.requestPrehandler)
    .all(this.entity())
    .get(this.entityGet())
    .post(this.entityPut())
    .put(this.entityPut())
    .delete(this.entityDelete())
};
